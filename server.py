...
# Webhook Credentials and settings
...
account_id = "17841472117168408"  # Replace with your actual account id
...
# Global dictionaries to hold conversation messages and scheduled tasks
message_queue = {}  # { conversation_id: [messages, ...] }
conversation_task_schedules = {}  # { conversation_id: asyncio.Task }
...
def analyze_sentiment(comment_text):
    ...
    if sentiment_scores['compound'] > 0.25:
        ...
        sentiment = "Negative"  # Treat neutral as negative for default responses
    return sentiment
...
async def process_dm(conversation_id: str, delay: int):
    """Async function to process DM after a delay."""
    try:
        await asyncio.sleep(delay)
        if conversation_id not in message_queue or not message_queue[conversation_id]:
            logger.info(f"No messages to process for conversation: {conversation_id}. Task exiting.")
            return

        messages = message_queue[conversation_id]
        recipient_id = messages[0]["sender_id"]
        combined_text = "\n".join([msg["text"] for msg in messages])
        sentiment = analyze_sentiment(combined_text)
        logger.info(f"Sentiment Analysis Result: {sentiment}, Combined Text: '{combined_text}'")
        if sentiment == "Positive":
            llm_prompt_suffix = ("Respond with a very enthusiastic and thankful tone, "
                                 "acknowledging the compliment. Keep it concise and friendly.")
        elif sentiment == "Negative":
            llm_prompt_suffix = ("Respond with an apologetic and helpful tone, asking for more details "
                                 "about the issue so we can improve. Keep it concise and professional.")
        else:
        ...
        try:
            response_text = await asyncio.to_thread(llm_response, gemini_api_key, model_name, full_prompt)
        except Exception as e:
            logger.error(f"Error generating LLM response: {e}")
            response_text = default_dm_response_positive if sentiment == "Positive" else default_dm_response_negative

        try:
            result = await asyncio.to_thread(postmsg, access_token, recipient_id, response_text)
            logger.info(f"Sent response to {recipient_id}. Result: {result}")
        except Exception as e:
            logger.error(f"Error sending message to {recipient_id}: {e}")

        # Clear the message queue and scheduled task for the conversation
        if conversation_id in message_queue:
            del message_queue[conversation_id]
            logger.info(f"Cleared message queue for conversation: {conversation_id}")
        if conversation_id in conversation_task_schedules:
            del conversation_task_schedules[conversation_id]

    except asyncio.CancelledError:
        logger.info(f"DM processing task for conversation {conversation_id} was cancelled.")
    except Exception as e:
        logger.error(f"Error in process_dm for conversation {conversation_id}: {e}")

async def process_comment_reply(access_token: str, comment_id: str, message_to_be_sent: str, delay: int):
    """Async function to send a reply to a comment after a delay."""
    try:
        await asyncio.sleep(delay)
        result = await asyncio.to_thread(sendreply, access_token, comment_id, message_to_be_sent)
...
async def webhook(request: Request):
    ...
    try:
        ...
        for event in parsed_events:
            ...
            if event["type"] == "direct_message" and not event.get("is_echo", False):
                conversation_id = f"{event['sender_id']}_{event['recipient_id']}"
                if conversation_id not in message_queue:
                    # New conversation
                    message_queue[conversation_id] = [event]
                    delay = random.randint(60, 120)  # 1 to 2 minutes delay
                    task = asyncio.create_task(process_dm(conversation_id, delay))
                    conversation_task_schedules[conversation_id] = task
                    logger.info(f"Scheduled initial DM task for conversation: {conversation_id}, delay: {delay}s")
                else:
                    ...
                    if conversation_id in conversation_task_schedules:
                        existing_task = conversation_task_schedules[conversation_id]
                        existing_task.cancel()  # Cancel the previously scheduled task
                        del conversation_task_schedules[conversation_id]
                        logger.info(f"Cancelled existing DM task for conversation: {conversation_id}")
                    new_delay = 30  # Shorter delay for re-scheduling (e.g., 30 seconds)
                    task = asyncio.create_task(process_dm(conversation_id, new_delay))
                    conversation_task_schedules[conversation_id] = task
                    logger.info(f"Re-scheduled DM task for conversation: {conversation_id}, new delay: {new_delay}s")
            elif event["type"] == "comment" and event.get("from_id") != account_id:
                ...
                delay = random.randint(60, 120)  # 1 to 2 minutes delay for reply
                asyncio.create_task(process_comment_reply(access_token, event["comment_id"], message_to_be_sent, delay))
